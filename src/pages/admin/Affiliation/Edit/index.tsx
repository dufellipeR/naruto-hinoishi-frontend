...
const EditAffiliation: React.FC = () => {
  const formRef = useRef<FormHandles>(null);

  const { user, signOut } = useAuth();
  const [affiliation, setAffiliation] = useState<AffiliationForm | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    api.get(`/affiliation/${id}/`).then((response) => {
  ...
  const handleSubmit = useCallback(
    async (data: AffiliationForm) => {
      try {
        ...
        await api.put(`/affiliation/${id}`, data);

        addToast({
          type: 'success',
          title: 'Successfully updated affiliation',
        });

        history.push('/admin/affiliation');
      } catch (err) {
