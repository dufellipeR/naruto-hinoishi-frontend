...
const EditKekkei: React.FC = () => {
  const formRef = useRef<FormHandles>(null);

  const { user, signOut } = useAuth();
  const [kekkei, setKekkei] = useState<KekkeiForm | undefined>(undefined);
  ...
  useEffect(() => {
    api.get(`/kekkei/${id}`).then((response) => {
      setKekkei(response.data);
  ...
  const handleSubmit = useCallback(
    async (data: KekkeiForm) => {
      try {
        ...
        await api.put(`/kekkei/${id}`, data);

        addToast({
          type: 'success',
          title: 'Kekkei Genkai atualizado com sucesso',
        });

        history.push('/admin/kekkeis');
      } catch (err) {
        ...
        addToast({
          type: 'error',
          title: 'Erro na criação do kekkei genkai',
          description:
            'Ocorreu um erro ao fazer a criação do kekkei genkai, cheque os dados informados',
        });
      }
    },
    [addToast, history],
  ...
  return (
    <Container>
      ...
      <Content>
        ...
        <Form ref={formRef} onSubmit={handleSubmit} initialData={kekkei}>
          <div>
